class RegistrationList
  ...
  def csv_for division, headers
  	...
  	to_csv headers do
  	  current = divisions[division[:key]].delete_if { |reg| reg['PlayUpRequest'] == 'Checked' }
  	  play_ups = divisions[division[:play_up_from]] ? divisions[division[:play_up_from][:key]].select { |reg| reg['PlayUpRequest'] == 'Checked' } : []
  	  (current + play_ups).sort_by { |reg| "#{reg[headers[0]]}|#{reg[headers[1]]}" }
  	end
  end
